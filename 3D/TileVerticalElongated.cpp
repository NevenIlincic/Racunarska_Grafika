...
TileVerticalElongated::TileVerticalElongated(unsigned int shader, std::vector<float> _downLeftVertex, std::vector<float> _normalVector, float _height, float _length, std::vector<float> _tileColor, bool _isNormalTowardsPositive):
    shaderProgram(shader), downLeftVertex(_downLeftVertex), normalVector(_normalVector), height(_height), length(_length), tileColor(_tileColor), isNormalTowardsPositive(_isNormalTowardsPositive)
{
